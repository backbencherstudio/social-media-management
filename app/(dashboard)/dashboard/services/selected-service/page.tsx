...
const ServiceSelectionPage = () => {
    ...
    const handleServiceSelect = (service: Service, isSelected: boolean, plan?: string) => {
        if (isSelected) {
            ...
            if (!exists) {
                ...
                setSelectedServices(selectedServices.map(item =>
                    item.service.id === service.id
                        ? { ...item, plan: plan || item.plan, price: plan ? calculatePlanPrice(service, plan) : item.price }
                        : item
                ));
            }
        } else {
            // Remove service
            setSelectedServices(selectedServices.filter(item => item.service.id !== service.id));
        }
    };

    const handlePlanChange = (serviceId: string, plan: string) => {
        setSelectedServices(selectedServices.map(item =>
            item.service.id === serviceId
                ? { ...item, plan, price: calculatePlanPrice(item.service, plan) }
                : item
        ));
    };

    const calculatePlanPrice = (service: Service, plan: string): number => {
        // This is a simplified calculation - in a real app, you'd have specific logic per service
        if (plan.includes('premium')) {
            return service.endingPrice;
        } else if (plan.includes('standard')) {
            return (service.startingPrice + service.endingPrice) / 2;
        }
            ...
            <div className='lg:w-[80%] w-full bg-white rounded-[12px] lg:p-6 md:p-4 p-3'>
                <div className='border-b pb-6 border-[#E9E9EA]'>
                    <p className='text-[#4A4C56] text-base leading-[150%] tracking-[.16px]'>
                        Affordable creative & marketing services, pay a fixed, monthly, and predictable rate, with no contracts or
                        surprises.
                    </p>
                    <p className='text-[#4A4C56] text-base leading-[150%] tracking-[.16px] italic mt-5'>
                        Note: Trying to sign up for multiple brands? If so, complete this order form for each unique brand. You will
                        be charged separately for each brand.
                    </p>
                </div>
                <div>
                    ...
                    <div className="space-y-6 mt-6">
                        {serviceOptions.map((service) => (
                            <ServiceOption
                                ...
                                isSelected={selectedServices.some(item => item.service.id === service.id)}
                                selectedPlan={selectedServices.find(item => item.service.id === service.id)?.plan || null}
                            />
                        ))}
                    </div>
                </div>
