...
const companies = [
    { id: "tagGrowth", name: "Tag Growth", plan: "Standard Plan", isHighlighted: true },
    { id: "buckSocial", name: "98 Buck Social", plan: "Starter", isHighlighted: false },
    { id: "smarcomms", name: "Smarcomms", plan: "Basic Plan", isHighlighted: false },
...
const socialIcons = {
    ...
    buckSocial: [<FacebookIcon className="w-[22px] h-[22px]" key="fb" />, <InstaIcon className="w-[24px] h-[24px]" key="ig" />, <YoutubeIcon className="w-[24px] h-[24px]" key="yt" />],
    smarcomms: [<FacebookIcon className="w-[22px] h-[22px]" key="fb" />, <InstaIcon className="w-[24px] h-[24px]" key="ig" />],
    socialSinQ: [<FacebookIcon className="w-[22px] h-[22px]" key="fb" />, <LinkdinIcon className="w-[24px] h-[24px]" key="ln" />, <InstaIcon className="w-[24px] h-[24px]" key="ig" />, <YoutubeIcon className="w-[24px]" key="yt" />],
}
                        ...
                        <TableHeader>
                            <TableRow className="leading-[150%] tracking-[.16px] lg:text-base text-sm">
                                ...
                                {companies.map((company) => (
                                    <TableHead
                                        key={company.id}
                                        className={`border border-gray-200  p-4 ${company.isHighlighted ? "text-white" : "bg-white text-[#1D1F2C]"
                                            }`}
                                        style={
                                            company.isHighlighted
                                                ? { background: "linear-gradient(110deg, #0D0D0D 5.15%, #7C7C85 139.45%)" }
                                                : undefined
                                        }
                                    >
                                        <div className="font-semibold lg:text-base text-sm">{company.name}</div>
                                        <div className="lg:text-base text-sm font-[400] ">{company.plan}</div>
                                    </TableHead>
                                ))}
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {features.map((feature) => (
                                <TableRow key={feature.id}>
                                    <TableCell className="border border-gray-200 p-4">{feature.name}</TableCell>
                                    {companies.map((company) => (
                                        <TableCell key={`${feature.id}-${company.id}`} className="border border-gray-200 p-4">
                                            {feature.isSpecial ? (
                                                <div className="flex items-center -space-x-[6px] ">
                                                    {feature.values[company.id as keyof typeof feature.values]}
                                                </div>
                                            ) : (
                                                feature.values[company.id as keyof typeof feature.values]
                                            )}
